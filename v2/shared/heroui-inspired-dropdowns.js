...
class LYDDropdownSystem {
    ...
    createPortalContainer() {
        ...
        this.portalContainer.id = 'core-dropdown-portal';
    ...
    setupDropdowns() {
        ...
        document.querySelectorAll('.core-select').forEach((select, index) => {
            const trigger = select.querySelector('.core-select-trigger');
            const dropdown = select.querySelector('.core-select-dropdown');
            
            if (!trigger || !dropdown) return;
            
            // Unique ID für Portal-Management
            const dropdownId = 'core-select-' + index;
            ...
            // Option-Handler
            dropdown.querySelectorAll('.core-select-option').forEach(option => {
        ...
        document.querySelectorAll('.core-dropdown').forEach((dropdown, index) => {
            const trigger = dropdown.querySelector('.core-dropdown-trigger');
            const menu = dropdown.querySelector('.core-dropdown-menu');
            
            if (!trigger || !menu) return;
            
            const menuId = 'core-dropdown-' + index;
            menu.id = menuId;
            ...
            // Menu-Item-Handler
            menu.querySelectorAll('.core-dropdown-item').forEach(item => {
        ...
        document.querySelectorAll('.core-autocomplete').forEach((autocomplete, index) => {
            const input = autocomplete.querySelector('.core-autocomplete-input');
            const dropdown = autocomplete.querySelector('.core-autocomplete-dropdown');
            
            if (!input || !dropdown) return;
            
            const dropdownId = 'core-autocomplete-' + index;
            dropdown.id = dropdownId;
            ...
            // Option-Handler
            dropdown.querySelectorAll('.core-autocomplete-item').forEach(item => {
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        document.addEventListener('click', (e) => {
            if (!e.target.closest('.core-select, .core-dropdown, .core-autocomplete, #core-dropdown-portal')) {
    ...
    setupClonedEventHandlers(clone, container, trigger, originalDropdown) {
        ...
        clone.querySelectorAll('.core-select-option, .core-dropdown-item').forEach(item => {
            item.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                
                // Text zum Trigger kopieren
                const selectedText = trigger.querySelector('.core-select-text, span:not(.core-dropdown-icon):not(.core-select-icon)');
    ...
    selectOption(select, trigger, dropdown, option) {
        ...
        // Selected state
        dropdown.querySelectorAll('.core-select-option').forEach(opt => {
    ...
    selectAutocompleteItem(container, input, dropdown, item) {
        ...
        // Selected state
        dropdown.querySelectorAll('.core-autocomplete-item').forEach(opt => {
            opt.classList.remove('selected');
        });
    ...
    filterAutocompleteOptions(dropdown, searchValue) {
        const items = dropdown.querySelectorAll('.core-autocomplete-item');
    ...
    setupAccessibility() {
        ...
        document.querySelectorAll('.core-select-trigger, .core-dropdown-trigger').forEach(trigger => {
